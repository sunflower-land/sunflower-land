...
import { FlowerFieldItem } from "features/game/types/game";
import { addNoise, RandomID } from "lib/images";

import { FLOWER_LIFECYCLE } from "../lib/flowerPlant";
import classnames from "classnames";
import { FLOWERS } from "features/game/types/flowers";

interface Props {
  flowerField?: FlowerFieldItem;
  className?: string;
  showFlowerDetails?: boolean;
}
...
export const FlowerSoil: React.FC<Props> = ({
  flowerField,
  className,
  showFlowerDetails,
}) => {
  ...
  const lifecycle = FLOWER_LIFECYCLE[flowerField.name];
  const timeLeft = getTimeLeft(flowerField.plantedAt, flower.harvestSeconds);

  // Seedling
  if (timeLeft > 0) {
    const percentage = 100 - (timeLeft / flower.harvestSeconds) * 100;
      ...
      <div className="relative w-full h-full">
        ...
        <InnerPanel
          className={classNames(
            ...
            {
              "opacity-100": showFlowerDetails,
              "opacity-0": !showFlowerDetails,
            }
          )}
        >

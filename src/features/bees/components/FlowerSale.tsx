...
import { FlowerName, FLOWERS, FLOWER_SEEDS } from "features/game/types/flowers";
...
import { getFlowerTime } from "features/game/events/getFlower";

interface Props {
  onClose: () => void;
}

export const FlowerSale: React.FC<Props> = ({ onClose }) => {
  const [selected, setSelected] = useState<Craftable>(
    FLOWER_SEEDS()["White Flower Seed"]
  );
  ...
  const flowerName = selected.name.split(" ")[0] as FlowerName;
  ...
  return (
    <div className="flex">
      <div className="w-3/5 flex flex-wrap h-fit">
        {Object.values(FLOWER_SEEDS()).map((item: Craftable) => (
          <Box
          ...
          <div className="border-t border-white w-full mt-2 pt-1">
            <div className="flex justify-center items-center">
              ...
              <span className="text-xs text-shadow text-center mt-2">
                {secondsToMidString(getFlowerTime(flower.name, inventory))}
              </span>
            </div>

import { millisecondsToString } from "lib/utils/time";
import { translate } from "lib/i18n/translate";
import {
  SKILL_RANKS,
  GOLDEN_SUNFLOWER_DISPLAY,
  type UpgradeableSkillName,
} from "features/game/types/bumpkinSkills";

/**
 * Human-readable boost description for a specific rank of an upgradeable skill.
 * Returns the buff sentence (and a debuff sentence for Acre/Hectare),
 * interpolating the real per-rank value from SKILL_RANKS so the panel can
 * show concrete numbers. `rank` is 1-based (1..maxLevel).
 */
export const getSkillRankDescription = (
  name: UpgradeableSkillName,
  rank: number,
): { buff: string; debuff?: string } => {
  const i = rank - 1;

  switch (name) {
    case "Green Thumb":
      return {
        buff: translate("skill.greenThumb.ranked", {
          value: SKILL_RANKS["Green Thumb"].ranks[i],
        }),
      };
    case "Strong Roots":
      return {
        buff: translate("skill.strongRoots.ranked", {
          value: SKILL_RANKS["Strong Roots"].ranks[i],
        }),
      };
    case "Young Farmer":
      return {
        buff: translate("skill.youngFarmer.ranked", {
          value: SKILL_RANKS["Young Farmer"].ranks[i],
        }),
      };
    case "Experienced Farmer":
      return {
        buff: translate("skill.experiencedFarmer.ranked", {
          value: SKILL_RANKS["Experienced Farmer"].ranks[i],
        }),
      };
    case "Old Farmer":
      return {
        buff: translate("skill.oldFarmer.ranked", {
          value: SKILL_RANKS["Old Farmer"].ranks[i],
        }),
      };
    case "Betty's Friend":
      return {
        buff: translate("skill.bettysFriend.ranked", {
          value: SKILL_RANKS["Betty's Friend"].ranks[i] * 100,
        }),
      };
    case "Coin Swindler":
      return {
        buff: translate("skill.coinSwindler.ranked", {
          value: SKILL_RANKS["Coin Swindler"].ranks[i] * 100,
        }),
      };
    case "Golden Sunflower":
      return {
        buff: translate("skill.goldenSunflower.ranked", {
          value: GOLDEN_SUNFLOWER_DISPLAY[i],
        }),
      };
    case "Chonky Scarecrow": {
      const { depth } = SKILL_RANKS["Chonky Scarecrow"].ranks[i];
      return {
        buff: translate("skill.chonkyScarecrow.ranked", {
          size: `${depth}x${depth}`,
        }),
      };
    }
    case "Horror Mike": {
      const { depth } = SKILL_RANKS["Horror Mike"].ranks[i];
      return {
        buff: translate("skill.horrorMike.ranked", {
          size: `${depth}x${depth}`,
        }),
      };
    }
    case "Laurie's Gains": {
      const { depth } = SKILL_RANKS["Laurie's Gains"].ranks[i];
      return {
        buff: translate("skill.lauriesGains.ranked", {
          size: `${depth}x${depth}`,
        }),
      };
    }
    case "Instant Growth":
      return {
        buff: translate("skill.instantGrowth.ranked", {
          value: millisecondsToString(SKILL_RANKS["Instant Growth"].ranks[i], {
            length: "short",
            isShortFormat: true,
            removeTrailingZeros: true,
          }),
        }),
      };
    case "Acre Farm":
      return {
        buff: translate("skill.acreFarm.buff.ranked", {
          value: SKILL_RANKS["Acre Farm"].buff[i],
        }),
        debuff: translate("skill.acreFarm.debuff.ranked", {
          value: SKILL_RANKS["Acre Farm"].debuff[i],
        }),
      };
    case "Hectare Farm":
      return {
        buff: translate("skill.hectareFarm.buff.ranked", {
          value: SKILL_RANKS["Hectare Farm"].buff[i],
        }),
        debuff: translate("skill.hectareFarm.debuff.ranked", {
          value: SKILL_RANKS["Hectare Farm"].debuff[i],
        }),
      };
  }
};

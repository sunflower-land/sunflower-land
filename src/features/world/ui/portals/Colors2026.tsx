...
export const Colors2026: React.FC<Props> = ({ onClose }) => {
  const { gameService } = useContext(Context);

  const minigames = useSelector(gameService, _minigames);
  const minigame = minigames.games["colors-2026"];
  const prize = minigames.prizes["colors-2026"];
  ...
  if (isPlaying) {
    return (
      <div>
        <Portal portalName="colors-2026" onClose={onClose} />
      </div>
    );
  }

  const onClaim = () => {
    gameService.send("minigame.prizeClaimed", { id: "colors-2026" });
      ...
      <div className="mb-1">
        <div className="p-2">
          <Label type="default" className="mb-1" icon={factions}>
            {t("colors2026.portal.title")}
          </Label>
          <InlineDialogue message={t("colors2026.portal.description")} />
        </div>

        <MinigamePrizeUI
          prize={prize}
          history={dailyAttempt}
          mission={t("colors2026.portal.missionObjectives")}
        />
      </div>
      <div className="flex">

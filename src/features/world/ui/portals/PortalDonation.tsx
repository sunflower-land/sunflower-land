    ...
    <>
      {state.matches("idle") && (
        <>
          <div className="p-1">
            ...
            <p className="text-sm">{`Would you like to donate ${donation.matic} MATIC to ${name}.`}</p>
          </div>
      ...
      {state.matches("confirming") && (
        <GameWallet action="donate">
          <p className="m-2">{`${donation.matic} (MATIC)`}</p>
      ...
      {state.matches("donated") && (
        <>
          <div className="p-2">{`Thanks for your donation!`}</div>

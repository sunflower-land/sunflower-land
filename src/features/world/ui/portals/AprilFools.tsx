...
export const AprilFools: React.FC<Props> = ({ onClose }) => {
  const { gameService } = useContext(Context);

  const minigames = useSelector(gameService, _minigames);
  const minigame = minigames.games["april-fools"];
  const prize = minigames.prizes["april-fools"];
  ...
  if (isPlaying) {
    return (
      <div>
        <Portal portalName="april-fools" onClose={onClose} />
      </div>
    );
  }

  const onClaim = () => {
    gameService.send("minigame.prizeClaimed", { id: "april-fools" });
      ...
      <div className="mb-1">
        <div className="p-2">
          <Label type="default" className="mb-1" icon={factions}>
            {t("aprilFools.portal.title")}
          </Label>
          <InlineDialogue message={t("aprilFools.portal.description")} />
        </div>

        <MinigamePrizeUI
          prize={prize}
          history={dailyAttempt}
          mission={t("aprilFools.portal.missionObjectives")}
        />
      </div>
      <div className="flex">

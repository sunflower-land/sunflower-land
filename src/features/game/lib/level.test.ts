...
describe("getBumpkinLevel", () => {
  it("returns level 6 if the player is 1 XP away from level 7", () => {
    const bumpkinExp = 3399;
    expect(Number(getBumpkinLevel(bumpkinExp))).toEqual(6);
  });
  it("returns level 7 if the player is 0 XP away from level 7", () => {
    const bumpkinExp = 4582;
    expect(Number(getBumpkinLevel(bumpkinExp))).toEqual(7);
  });
  it("returns level 7 if the player is 1 XP above level 7", () => {
...
describe("isMaxLevel", () => {
  it("returns false if 1 XP away from max level", () => {
    const bumpkinExp = 89999;
    expect(isMaxLevel(bumpkinExp)).toBeFalsy();
  });
  it("returns false if 0 XP away from max level", () => {
    const bumpkinExp = 220500;
    expect(isMaxLevel(bumpkinExp)).toBeTruthy();
  });
  it("returns false if 1 XP above max level", () => {

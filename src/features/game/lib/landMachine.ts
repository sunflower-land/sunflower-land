...
export const landMachine = createMachine<
  Context,
  BlockchainEvent,
  BlockchainState
>(
  {
    ...
    states: {
      loading: {
        invoke: {
          src: async (context) => {
            const farmId = context.farmId as number;

            const { game: onChain, owner } = await getOnChainState({
              farmAddress: context.address as string,
              id: farmId,
            });
            ...
            if (sessionId) {
              ...
              // add farm address
              game.farmAddress = context.address;
          ...
          src: (context) => (cb) => {
            const interval = setInterval(async () => {
              const sessionID = await metamask
                .getSessionManager()
                ?.getSessionId(context?.farmId as number);

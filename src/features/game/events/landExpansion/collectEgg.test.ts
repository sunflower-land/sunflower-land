...
describe("collect eggs", () => {
  ...
  it("throws an error if chicken does not exist", () => {
    expect(() =>
      collectEgg({
        state: GAME_STATE,
        action: { type: "chicken.eggCollected", index: 3 },
      })
  ...
  it("throws an error if chicken hasn't layed egg", () => {
    expect(() =>
      collectEgg({
        ...
        action: { type: "chicken.eggCollected", index: 0 },
      })
  ...
  it("can collect an egg multiple times", () => {
    ...
    const stateAfterFirstEggCollected = collectEgg({
      state,
      action: { type: "chicken.eggCollected", index: 0 },
    });

    const stateAfterSecondEggCollected = collectEgg({
      state: stateAfterFirstEggCollected,
      action: { type: "chicken.eggCollected", index: 1 },
    });

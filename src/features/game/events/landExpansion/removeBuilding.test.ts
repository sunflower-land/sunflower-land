...
describe("removeBuilding", () => {
  ...
  it("does not remove Pet House when pets are placed inside", () => {
    expect(() =>
      removeBuilding({
        state: {
          ...GAME_STATE,
    ...
    ).toThrow(REMOVE_BUILDING_ERRORS.PET_HOUSE_HAS_PETS);
  });

  it("allows removing Pet House when no pets are placed inside", () => {
    const dateNow = Date.now();
    const state = removeBuilding({
      state: {
        ...GAME_STATE,
    ...
    expect(state.buildings["Pet House"]?.[0].coordinates).toBeUndefined();
    expect(state.buildings["Pet House"]?.[0].removedAt).toEqual(dateNow);
  });

  it("allows removing Pet House when all pets have been removed (no coordinates)", () => {
    const dateNow = Date.now();
    const state = removeBuilding({
      state: {
        ...GAME_STATE,

...
describe("getPriceHistory", () => {
  ...
  it("fills gaps from the next older day and avoids Infinity/NaN when the first trade is within the window", () => {
    const from = new Date("2024-12-11T00:00:00.000Z").getTime();

    // Only last few days have trades; older dates are implicitly 0.
    const dates: Record<string, ReturnType<typeof buildDay>> = {
      "2025-01-08": buildDay({ date: "2025-01-08", low: 50 }),
      "2025-01-09": buildDay({ date: "2025-01-09", low: 0 }), // explicit gap
      "2025-01-10": buildDay({ date: "2025-01-10", low: 100 }),
    };
    ...
    // Gap fill: Jan 9 should inherit from the older day (Jan 8), not newer (Jan 10)
    ...
    expect(Number.isFinite(result.sevenDayChange)).toBe(true);
  });

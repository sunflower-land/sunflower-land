...
const SEASONAL_REWARDS: (weight: number) => ChestReward[] = (weight) => {
  ...
  if (isPharaohSeason) {
    return [
      ...
      { items: { Cannonball: 1 }, weighting: weight / 10 },
      { items: { Sarcophagus: 1 }, weighting: weight / 3 },
      { items: { "Clay Tablet": 1 }, weighting: weight },
      { items: { "Snake in Jar": 1 }, weighting: weight / 5 },
      { items: { "Reveling Lemon": 1 }, weighting: weight / 10 },
      { items: { "Anubis Jackal": 1 }, weighting: weight / 3 },
      { items: { Sundial: 1 }, weighting: weight / 2 },
      { items: { "Sand Golem": 1 }, weighting: weight / 4 },
      { items: { "Cactus King": 1 }, weighting: weight / 5 },
      { items: { "Lemon Frog": 1 }, weighting: weight / 10 },
      { items: { "Scarab Beetle": 1 }, weighting: weight / 3 },
      { items: { "Golden Garrison": 1 }, weighting: weight },
      { items: { "Rookie Rook": 1 }, weighting: weight },
      { items: { "Silver Sentinel": 1 }, weighting: weight },
      { items: { "Sunlit Citadel": 1 }, weighting: weight / 25 },
      { items: { Camel: 1 }, weighting: weight / 60 },
      { items: { "Baobab Tree": 1 }, weighting: weight / 10 },
      { wearables: { "Grape Pants": 1 }, weighting: weight / 2 },
      { wearables: { "Amber Amulet": 1 }, weighting: weight / 2 },
      { wearables: { "Explorer Shirt": 1 }, weighting: weight / 4 },
      { wearables: { "Crab Trap": 1 }, weighting: weight / 10 },
      { wearables: { "Water Gourd": 1 }, weighting: weight / 3 },
      { wearables: { "Ankh Shirt": 1 }, weighting: weight / 2 },
      { wearables: { "Explorer Shorts": 1 }, weighting: weight / 3 },
      { wearables: { "Explorer Hat": 1 }, weighting: weight / 4 },
      { wearables: { "Desert Camel Background": 1 }, weighting: weight },
      { wearables: { "Rock Hammer": 1 }, weighting: weight / 3 },
      { wearables: { "Elf Potion": 1 }, weighting: weight },
      { wearables: { "Scarab Wings": 1 }, weighting: weight / 45 },
    ];
  } else {
    return [];
  }

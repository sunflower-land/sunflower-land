...
export const frogMachine = createMachine<Context, FrogEvent, FrogState>(
  {
    ...
    states: {
      loading: {
        invoke: {
          src: async () => {
            const frogCount = await metamask.getFrog().getTotalSupply();
            console.log(frogCount);
            const isWhitelistEnabled = await metamask
              .getFrog()
              .isWhitelistingEnabled();
            console.log(isWhitelistEnabled);
            const isWalletBlacklisted = await metamask
              .getFrog()
              .isWalletBlacklisted();
      ...
      check_whitelist: {
        invoke: {
          src: async () => {
            const isWalletWhitelisted = await metamask
              .getFrog()
              .isWalletWhitelisted();

...
const flowerBed: Record<FlowerBed, string> = {
  ...
  "flowerBed.newSpecies.superLucky":
    "Ого! Сегодня ваш счастливый день, ты сделал два открытия за один раз! Ты открыл новый вид цветка, и появился дополнительный мутантный цветок",
...
const guideTerms: Record<GuideTerms, string> = {
  ...
  "crops.guide.one":
    "В Sunflower Land посевы играют решающую роль на вашем пути к процветанию. Сажая и собирая урожай, ты можешь зарабатывать монеты или использовать их для создания ценных рецептов и предметов в игре.",
  "crops.guide.two":
    "Чтобы вырастить культуры, вам нужно приобрести соответствующие семена во внутриигровом магазине. У каждой культуры разное время роста: от 1 минуты для Sunflower до 36 часов для Kale. Когда культуры полностью вырастут, ты сможешь собрать урожай и извлечь из него пользу.",
  "crops.guide.three":
    "Помните, что по мере того, как ты будешь расширять свои угодья и продвигаться по игре, вам будет доступно все больше культур, что откроет новые возможности для заработка монет и изучения огромного потенциала фермерской экономики Sunflower Land. Не бойтесь испачкать руки, сажайте семена и наблюдайте за тем, как расцветают ваши посевы, собирая урожай на пути к успеху!",
  ...
  "cooking.guide.one":
    "Приготовление пищи позволяет вам накормить своего бампкина и помочь ему получить ценные очки опыта (XP). Используя собранный урожай, ты можешь приготовить вкусную еду в различных зданиях, предназначенных для ее приготовления.",
  "cooking.guide.two":
...
const onboarding: Record<Onboarding, string> = {
  ...
  "onboarding.intro.one":
    "В своих путешествиях ты будешь получать редкие NFT, которые нужно защищать. Чтобы сохранить их в безопасности, вам понадобится кошелек Web3.",
...
const tieBreaker: Record<TieBreaker, string> = {
  ...
  "tieBreaker.closeBid":
    "В этом случае артефакт уходит тому, у кого больше опыта. К сожалению, вы проиграли.",
...
const transactionTerms: Record<TransactionTerms, string> = {
  ...
  "transaction.displayItems":
    "Обратите внимание, что OpenSea может потребоваться до 30 минут, чтобы отобразить твои товары. Вы также можете просмотреть свои товары на",
...
const milestoneMessages: Record<MilestoneMessages, string> = {
  ...
  "milestone.masterAngler":
    "Вау, ты только что достиг отметки Master Angler! Поймать 1500 рыб - это свидетельство вашего мастерства рыболова.",
  "milestone.marineMarvelMaster":
    "Поздравляем, ты только что достиг отметки Marine Marvel Master! Ты - бесспорный чемпион морей! Поимка каждого Marvel доказывает ваше рыболовное мастерство, как ничто другое.",
  "milestone.deepSeaDiver":
    "Поздравляем, ты только что достиг отметки Deep Sea Diver! Ты заслужил Deep Sea Helm - таинственную корону, которая привлекает Marvel'ов на ваш крючок.",
...
const cropMachine: Record<CropMachine, string> = {
  ...
  "cropMachine.readyCropPacks.description":
    "В настоящее время у вас есть {{totalReady}} пакетов для сбора урожая! Щелкните на пакет, в который ты хочешь собрать.",
...
const removeCropMachine: Record<RemoveCropMachine, string> = {
  ...
  "removeCropMachine.description":
    "Это действие удалит всю нефть, хранящуюся в вашей сельскохозяйственной машине.",
};

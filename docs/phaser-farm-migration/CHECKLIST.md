...
- [x] Parity round 2026-08-29: crop-machine stage sheets + ready overlay + icon row ✅; hen-house/barn alert rows ✅; house extras ✅ (daily chest → DAILY_REWARD, HomeBumpkins row, LetterBox modal via refactored `LetterBoxModals`, collect heart via `bridge.onGameEvent`); market sale labels + first-sale helpers ✅; water-well auto-open on `building.upgraded` ✅. Tent bumpkin = N/A (dead code — DOM's walletBumpkins is hardcoded `[]`). _Still deferred: fish-market idle Neville (composed bumpkin)._ **GIF statics resolved 2026-08-30** — see the animated-art entry in Phase 10.
...
- [x] Stateful collectibles ✅ (2026-08-29): reveals (Genie Lamp confirm→REVEAL→genieRevealed, Maneki Neko canShake→REVEAL, Festive Tree season gate; `overlay/RevealModals.tsx`), Monuments (staged PROJECT_IMAGES art + quantity cheer bar + completion modal + cheers popover), Beds (staged art + unlock-farmhand modal), Salt Sculpture (leveled art + upgrade modal), Obsidian Shrine + all pet shrines (extended EXPIRING config: activeBar/renewFlow/alertTop + `renewPetShrine`/`obsidianShrine` modals), Bush season variants, Wicker Man/Tomato Bombard click-burst sheets + Rock Golem state sheets (`CLICK_SHEETS` + `renderRockGolem`), SFT detail popovers on EVERY collectible + buds + active boosts (`bridge.sftPopover` + `overlay/SftPopoverUI.tsx`), ScarecrowAOEOverlay in landscaping (`drawAoeOverlays`, shared getAOEExtent). _Still deferred: weather-protection `used` alerts._ **GIF statics resolved 2026-08-30** — see the animated-art entry in Phase 10.
...
- [x] Parity round 2026-08-29: auras ✅ (back/front 20×19 8f 14fps in NPCSprite), tutorial click helpers ✅ (plots dig/click, market, PlayerNPC), bud click popover ✅ (via sftPopover). N/A-by-DOM: pet XP floats (DOM doesn't render them), NFT pet walking (walking pref unconsumed). _Still deferred: PlayerModal/Discovery singletons on visit; the 0.78px vertical delta_

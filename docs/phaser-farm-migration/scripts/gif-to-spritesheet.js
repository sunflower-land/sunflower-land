...
const MANIFEST = path.join(REPO, "src/features/farmEngine/core/animatedArt.ts");
  ...
  const walk = (d) => {
    for (const entry of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        fs.closeSync(fd);
        if (head.includes("ANIM"))
          out.push(path.relative(path.join(REPO, "src"), full));
      }
    }
  };
  walk(dir);
...
/**
 * Nominal frameRate for the animation — the median delay. This alone is what
 * the manifest used to carry, which silently retimed any art whose frames
 * don't all hold for the same length: Fat Chicken's [70,70,140,280,70] (630ms
 * a loop) collapsed to 14fps and ran 350ms, losing its long pose and playing
 * ~1.8x too fast, while El Pollo Veloz's median pushed it too slow. Phaser
 * keeps the frameRate as the fallback tick and honours a per-frame `duration`
 * as that frame's absolute tick, so `delaysOf` below preserves the real
 * timing and this stays the baseline.
 */
...
/**
 * Per-frame delays in ms, but only when they actually vary — uniform art is
 * fully described by its frameRate and doesn't need to carry the array.
 */
const delaysOf = (delays) => {
  const list = (delays ?? []).filter((d) => d > 0);
  if (list.length < 2) return undefined;
  return new Set(list).size > 1 ? list : undefined;
};
...
(async () => {
  ...
  for (const source of LOCAL) {
    ...
    if (!result) {
      skipped.push(
        `${source} (strip exceeds ${MAX_SHEET_DIM}px — add to EXCLUDED)`,
      );
      continue;
    }
  ...
  const importLines = [
    ...new Set([
      ...
      ...remoteEntries.map(
        (e) =>
          `import ${varName(e.name)} from "assets/animations/${e.name}.png";`,
      ),
      ...tokenEntries.map(
        (e) =>
          `import ${varName(e.name)} from "assets/animations/${e.name}.png";`,
  ...
  const entryLine = (e, keyExpr) =>
    `  [${keyExpr}, { sheet: ${varName(e.name)}, frameWidth: ${e.frameWidth}, frameHeight: ${e.frameHeight}, frames: ${e.frames}, fps: ${e.fps}${e.delays ? `, delays: [${e.delays.join(", ")}]` : ""} }],`;
...
export type AnimatedArt = {
  ...
  frameHeight: number;
  frames: number;
  /** Nominal rate (median delay) — the tick for any frame without a delay. */
  fps: number;
  /**
   * Per-frame hold times in ms, present only when the source's frames don't
   * all hold for the same length. Phaser treats a frame's \`duration\` as its
   * absolute tick, so this reproduces the GIF/WebP timing exactly instead of
   * flattening it to one rate.
   */
  delays?: number[];
};
...
${remoteEntries
  .map((e) => entryLine(e, `\`\${CONFIG.PROTECTED_IMAGE_URL}${e.cdnPath}\``))
  .join("\n")}
${tokenEntries
...
`;
  ...
  // The manifest is linted like any other source file, and one entry per line
  // blows past the print width — format it here so a regenerate never leaves
  // the tree failing `yarn lint`.
  let formatted = body;
  try {
    const prettier = require("prettier");
    const config = (await prettier.resolveConfig(MANIFEST)) ?? {};
    formatted = await prettier.format(body, { ...config, filepath: MANIFEST });
  } catch (error) {
    console.log(
      `\nWARNING: could not prettier-format the manifest: ${error.message}`,
    );
  }
  fs.writeFileSync(MANIFEST, formatted);
  console.log(
...
function varName(name) {
  return name.replace(/[^a-zA-Z0-9]+(.)?/g, (_, c) =>
    c ? c.toUpperCase() : "",
  );
}
